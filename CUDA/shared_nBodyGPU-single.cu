...
__global__ void calculate_forces(float4* _p, float4* _f, float* _m, int N)
{
	...
	float m = _m[index];
	float4 f = _f[index]; float4 pos = _p[index];
...
int main(int argc, char* argv[])
{
	...
	{
		calculate_forces << <dim3(N / BLOCK_SIZE), dim3(BLOCK_SIZE) >> > (dev_p, dev_f, dev_m, N);
		move_particles << <dim3(N / BLOCK_SIZE), dim3(BLOCK_SIZE) >> > (dev_p, dev_v, dev_f, dev_m, N);
		//calculate_forces << <dim3((N + BLOCK_SIZE - 1) / BLOCK_SIZE), dim3(BLOCK_SIZE) >> > (dev_p, dev_v, dev_f, dev_m, N);
		//move_particles << <dim3((N + BLOCK_SIZE - 1) / BLOCK_SIZE), dim3(BLOCK_SIZE) >> > (dev_p, dev_v, dev_f, dev_m, N);
	}
